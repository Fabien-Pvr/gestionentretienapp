...
const countElementsBesoinEntretien = async () => {
  try {
    ...
    console.error('Erreur lors de la récupération du nombre d\'éléments :', error.message);
    return 0;
  }
};
...
const NoticeForm = () => {
  ...
  return (
    <form onSubmit={handleFormSubmit}>
      ...
      <label>
        Type d'entretien:
        <input type="text" value={TypeEntretien} onChange={(e) => setTypeEntretien(e.target.value)} />
      </label>

      <label>
        Périodicité de l'entretien :
        <input type="number" value={Periodicite} onChange={(e) => setPeriodicite(e.target.value)} />
      </label>
      <label>
        Capacité : 
        <input
        ...
        <input type="text" value={TypeHuile} onChange={(e) => setTypeHuile(e.target.value)} />
          </label>
          <label>
        Nombre de filtre nécessaire :
        <input type="number" value={NbFiltre} onChange={(e) => setNbFiltre(e.target.value)} />
      </label>
      <label>
        Référence du premier Filtre :
        <input type="text" value={RefFiltre1} onChange={(e) => setRefFiltre1(e.target.value)} />
      </label>
